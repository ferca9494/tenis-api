...
export class PartidoService {
    ...
    private gameLocal(partido: Partido) {
        ...
        if (partido.cantidadGamesLocal == 6) {
            ...
            //punto 1.3
            this.rankJugador(partido,false);
    ...
    private gameVisitante(partido: Partido) {
        ...
        if (partido.cantidadGamesVisitante == 6) {
            ...
            this.rankJugador(partido,true);
        }
    }

    private rankJugador(par : Partido , quiengano: boolean){

        // false = local
        // true = visitante

        const p_ganaLocal = 10;
        const p_ganaVisitante = 15;
        const p_pierdeLocal = -5;
        const p_pierdeVisitante = 0;

        if (quiengano) {
            par.jugadorVisitante.puntos += p_ganaVisitante;
            par.jugadorLocal.puntos += p_pierdeLocal;
        }else{
            par.jugadorVisitante.puntos += p_pierdeVisitante;
            par.jugadorLocal.puntos += p_ganaLocal;
        }
    }
}
